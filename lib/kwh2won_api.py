...
def _relativedelta_months(dt, months):
    """dateutil.relativedelta 대체: 월 단위 이동"""
...
class kwh2won_api:
    ...
    def __init__(self, pressure='low', checkDay=1, today=None, bigfamDcCfg=0, welfareDcCfg=0, rates=None):
        if rates is None:
            raise ValueError("요금 정보(rates)가 제공되지 않았습니다. RatesManager를 통해 먼저 로드하세요.")
        self.RATES = rates
    ...
    def prev2_checkday(self, today):
        prev = self.prev_checkday(today)
        return _relativedelta_months(prev, -1)
